...
pdf('out/alphaVsBeta.pdf',width=8,height=12)
  layout(matrix(c(0,1,1,0,rep(2:7,each=2)),nrow=4,byrow=TRUE))
  ifnaRange<-range(hiv$IFNa2.Pooled.Donor.cells.IC50..pg..ml)
  ifnbRange<-range(hiv$IFNbeta.Pooled.Donor.cells.IC50..pg.ml)
  ...
  segments(ifnaRange[1],ifnbRange[1],ifnaRange[2],ifnbRange[2])
  logAxis(1)
  logAxis(2,las=1)
  legend('topleft',names(cols),pch=21,pt.bg=cols,inset=.01,title='Pair')
  for(ii in unique(hiv$select)){
    withAs(xx=hiv[hiv$select==ii&hiv$donor,],plot(xx$IFNa2.Pooled.Donor.cells.IC50..pg..ml,xx$IFNbeta.Pooled.Donor.cells.IC50..pg.ml,log='xy',xaxt='n',yaxt='n',xlab='IFNa2 IC50 (pg/ml)',ylab='IFNbeta IC50 (pg/ml)',bg=cols[as.character(xx$Pair.ID)],pch=21,col=NA,main=sprintf('Donor Treat:%s',ii),xlim=ifnaRange,ylim=ifnbRange))
    logAxis(1)
    logAxis(2,las=1)
    withAs(xx=hiv[hiv$select==ii&!hiv$donor,],plot(xx$IFNa2.Pooled.Donor.cells.IC50..pg..ml,xx$IFNbeta.Pooled.Donor.cells.IC50..pg.ml,log='xy',xaxt='n',yaxt='n',xlab='IFNa2 IC50 (pg/ml)',ylab='IFNbeta IC50 (pg/ml)',bg=cols[as.character(xx$Pair.ID)],pch=21,col=NA,main=sprintf('Recipient Treat:%s',ii),xlim=ifnaRange,ylim=ifnbRange))
    logAxis(1)
    logAxis(2,las=1)
  }
dev.off()
