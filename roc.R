...
pdf('out/roc.pdf',width=4.5,height=4.5)
  for(genital in unique(hiv$isGenital)){
    ...
    sensCI<-lapply(goodTargetCols,function(xx)ci.se(roc(!hiv[thisSelect,'isDonor'],hiv[thisSelect,xx]),specificities=seq(0,1,.01)))
  ...
  for(treat in unique(hiv$Selection[hiv$Selection!='UT'])){
    thisSelect<-selector&((hiv$isDonor&hiv$Selection==treat)|(!hiv$isDonor&hiv$Selection=='UT'))&!hiv$isGenital
    preds<-lapply(goodTargetCols,function(xx)prediction(hiv[thisSelect,xx], !hiv[thisSelect,'isDonor']))
    names(preds)<-goodTargetCols
    perfs<-lapply(preds,function(pred)performance(pred, "tpr", "fpr"))
    aucs<-lapply(preds,function(pred)performance(pred, "auc")@y.values[[1]])
