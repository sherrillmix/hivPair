...
pdf('out/roc.pdf',width=4.5,height=4.5)
  ...
  genital<-FALSE
  for(ii in 1:length(goodTargetCols)){
    message(ifelse(genital,'Genital','Plasma'))
    thisSelect<-selector&hiv$select=='UT'&(hiv$isGenital==genital|!hiv$donor)
    preds<-lapply(names(goodTargetCols[1:ii]),function(xx)prediction(hiv[thisSelect,xx], !hiv[thisSelect,'donor']))
    aucCI<-lapply(names(goodTargetCols[1:ii]),function(xx)ci(!hiv[thisSelect,'donor'],hiv[thisSelect,xx]))
    sensCI<-lapply(names(goodTargetCols[1:ii]),function(xx)ci.se(roc(!hiv[thisSelect,'donor'],hiv[thisSelect,xx]),specificities=seq(0,1,.01),parallel=TRUE))
    names(preds)<-names(goodTargetCols[1:ii])
    perfs<-lapply(preds,function(pred)performance(pred, "tpr", "fpr"))
    aucs<-lapply(preds,function(pred)performance(pred, "auc")@y.values[[1]])
    print(aucs)
    print(aucCI)
    ...
    cols2<-rainbow.lab(length(perfs),alpha=.3)
    par(mar=c(3.5,3.5,1,.1))
    plot(1,1,type='n',xlim=0:1,ylim=0:1,xlab=perfs[[1]]@x.name,ylab=perfs[[1]]@y.name,las=1,mgp=c(2.2,.5,0))
    for(ii in 1:length(perfs))lines(perfs[[ii]]@x.values[[1]],perfs[[ii]]@y.values[[1]],col=cols[[ii]],lw=3)
    for(ii in 1:length(perfs))polygon(1-c(seq(0,1,.01),rev(seq(0,1,.01))),c(sensCI[[ii]][,'2.5%'],rev(sensCI[[ii]][,'97.5%'])),col=cols2[[ii]],border=NA)
    abline(0,1,lty=2)
    legend('bottomright',sub('\\(.*$','',targetCols[names(perfs)]),col=cols,lwd=2,adj=c(0,.5),bty='n')
...
logAxis(2,las=1)
withAs(xx=hiv[hiv$select=='UT'&selector&!hiv$isGenital,],vpPlot(ifelse(xx$donor,'Donor','Recipient'),xx$Infectivity.RLU.pg.RT...T1249,offsetXArgs=list(width=.2),ylab='Infectivity',las=1,yaxt='n',pch=21,bg=ifelse(xx$donor,'#FF000033','#0000FF33'),col='#00000033',log='y',mgp=c(2.5,1,0)))
logAxis(2,las=1)
dev.off()
